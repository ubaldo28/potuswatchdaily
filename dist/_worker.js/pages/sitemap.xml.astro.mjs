...
const GET = async ({ locals }) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from("articles").select("slug,title,hero_image,image,date,published_at").order("id", { ascending: false }).limit(1e3);
    ...
    const latestModified = articles[0] ? isoOf(articles[0]) : (/* @__PURE__ */ new Date()).toISOString();
    const staticUrls = [
